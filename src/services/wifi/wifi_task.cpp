...
static void saveWifiConfig() {
    if (wifiConfigMutex != NULL && xSemaphoreTake(wifiConfigMutex, portMAX_DELAY) == pdTRUE) {
        {
            core_nvs_save_string("wifi", "ssid", wifi_ssid);
            core_nvs_save_string("wifi", "pass", wifi_password);
            core_nvs_save_string("wifi", "client_id", wifi_client_id);
            core_nvs_save_string("wifi", "ip", wifi_ip);
            core_nvs_save_string("wifi", "gw", wifi_gateway);
            core_nvs_save_string("wifi", "sn", wifi_subnet);
            core_nvs_save_string("wifi", "dns", wifi_dns1);
            /* lock released */
        }
...
static void loadWifiConfig() {
    initWifiMutex();
    core_nvs_register_namespace("wifi");

    if (wifiConfigMutex != NULL && xSemaphoreTake(wifiConfigMutex, portMAX_DELAY) == pdTRUE) {
        {
            wifi_ssid      = core_nvs_read_string("wifi", "ssid", WIFI_SSID);
            wifi_password  = core_nvs_read_string("wifi", "pass", WIFI_PASSWORD);
            wifi_client_id = core_nvs_read_string("wifi", "client_id", WIFI_CLIENT_ID);
            wifi_ip        = core_nvs_read_string("wifi", "ip", STATIC_IP);
            wifi_gateway   = core_nvs_read_string("wifi", "gw", STATIC_GATEWAY);
            wifi_subnet    = core_nvs_read_string("wifi", "sn", STATIC_SUBNET);
            wifi_dns1      = core_nvs_read_string("wifi", "dns", STATIC_DNS1);
            /* lock released */
        }
        xSemaphoreGive(wifiConfigMutex);
    }
