...
static void saveLogConfig() {
    {
        core_nvs_save_int("log", "level", (int32_t)core_log_get_level());
        /* lock released */
    }
}
...
static void loadLogConfig() {
    core_nvs_register_namespace("log");
    {
        int32_t savedLevel = core_nvs_read_int("log", "level", (int32_t)LOG_LEVEL_DEBUG);
        if (savedLevel >= (int32_t)LOG_LEVEL_DEBUG && savedLevel <= (int32_t)LOG_LEVEL_ERROR) {
            core_log_set_level((LogLevel)savedLevel);
        }
        /* lock released */
    }
}
...
static void setLogLevel(const String &arg) {
    ...
    LogLevel level = core_log_get_level(); // Default to existing level if invalid
    ...
    core_log_set_level(level);
    saveLogConfig();
    LOG_INFO("Set Log Level: " + String(core_log_get_level()) + " (" + levelToStr(core_log_get_level()) + ")");
}

/** @brief Command handler: Prints current log severity level. */
static void getLogLevel(const String &arg) {
    LOG_INFO("Current Log Level: " + String(levelToStr(core_log_get_level())));
}
 ...
 */
static void websocket_log_cb(const String &msg, LogLevel level) {
...
void log_task_init(void) {
    core_log_init();
    core_log_register_cb(websocket_log_cb);
...
void vLogTask(void *pvParameters) {
    ...
    loadLogConfig();
    setSerialLogReady();
    LOG_INFO("Init log task done. Current level: " + levelToStr(core_log_get_level())); 
    ...
    for (;;) {
#if !(defined(ARDUINO_USB_CDC_ON_BOOT) && (ARDUINO_USB_CDC_ON_BOOT > 0))
        /* In case use UART, the callback is can't run in background by interupt, need to check manually per 50 ms */
        processHardwareSerialInput(); // Check the uart recive buffer and post into command Queue if command come in
