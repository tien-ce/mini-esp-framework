...
static void saveSensorConfig() {
    ...
    if (configMutex != NULL && xSemaphoreTake(configMutex, portMAX_DELAY) == pdTRUE) {
        {
            core_nvs_save_string("sensor_drv", "clientId", sensor_client_id);
            core_nvs_save_string("sensor_drv", "apiUrl", sensor_api_url);
            /* lock released */
        }
...
static void loadSensorConfig() {
    initSensorMutex();
    core_nvs_register_namespace("sensor_drv");

    if (configMutex != NULL && xSemaphoreTake(configMutex, portMAX_DELAY) == pdTRUE) {
        {
            sensor_client_id = core_nvs_read_string("sensor_drv", "clientId", E3FR2C1_COUNT_CLIENT_ID);
            sensor_api_url = core_nvs_read_string("sensor_drv", "apiUrl", E3FR2C1_COUNT_API_URL);
            /* lock released */
        }
        xSemaphoreGive(configMutex);
    }
